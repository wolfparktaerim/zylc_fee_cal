...
var ExpressionEntity = class {
  ...
  includeCallArguments(context, interaction) {
    for (const argument of interaction.args) {
      argument == null ? void 0 : argument.includePath(UNKNOWN_PATH, context, false);
    }
  }
...
var NodeBase = class extends ExpressionEntity {
  ...
  includePath(_path, context, includeChildrenRecursively, _options) {
    ...
    for (const key of childNodeKeys[this.type]) {
      ...
      if (Array.isArray(value)) {
        for (const child of value) {
          child == null ? void 0 : child.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
        }
      } else {
    ...
    if (key == null || includeChildrenRecursively) {
      for (const property2 of this.allProperties) {
        if (includeChildrenRecursively || property2.shouldBeIncluded(context)) {
          property2.includePath(EMPTY_PATH, context, includeChildrenRecursively);
        }
      }
      (_a = this.prototypeExpression) == null ? void 0 : _a.includePath(EMPTY_PATH, context, includeChildrenRecursively);
    } else {
      ...
      for (const property2 of includedMembers) {
        property2.includePath(includedPath, context, includeChildrenRecursively);
      }
      (_b = this.prototypeExpression) == null ? void 0 : _b.includePath(path, context, includeChildrenRecursively);
    }
  }
...
var LocalVariable = class extends Variable {
  constructor(name, declarator, init2, initPath, context, kind) {
    super(name);
    this.init = init2;
  ...
  includePath(path, context) {
    ...
    if (!this.includedPathTracker.includePathAndGetIfIncluded(path)) {
      ...
      if (path.length > 0) {
        ...
        (_a = this.additionalInitializers) == null ? void 0 : _a.forEach((initializer) => initializer.includePath(UNKNOWN_PATH, context, false));
      }
    }
  }
  includeCallArguments(context, interaction) {
...
var Identifier2 = class extends IdentifierBase {
  ...
  hasEffectsWhenDestructuring(context, destructuredInitPath, init2) {
    return destructuredInitPath.length > 0 && init2.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, context);
  }
  includeDestructuredIfNecessary(context, destructuredInitPath, init2) {
    ...
    const { propertyReadSideEffects } = this.scope.context.options.treeshake;
    if (this.included || (this.included = destructuredInitPath.length > 0 && !context.brokenFlow && propertyReadSideEffects && (propertyReadSideEffects === "always" || init2.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, createHasEffectsContext())))) {
      if (this.variable && !this.variable.included) {
        this.scope.context.includeVariableInModule(this.variable, EMPTY_PATH);
      }
      init2.includePath(destructuredInitPath, context, false);
      return true;
    }
    return false;
  }
...
var Scope = class {
   ...
   */
  addDeclaration(identifier2, context, init2, destructuredInitPath, kind) {
    var _a;
    ...
    const newVariable = new LocalVariable(identifier2.name, identifier2, init2, destructuredInitPath, context, kind);
...
var BlockScope = class extends ChildScope {
  ...
  addDeclaration(identifier2, context, init2, destructuredInitPath, kind) {
    ...
    if (kind === "var") {
      ...
      const declaredVariable = this.parent.addDeclaration(identifier2, context, init2, destructuredInitPath, kind);
      declaredVariable.markInitializersForDeoptimization();
      this.addHoistedVariable(name, declaredVariable);
      return declaredVariable;
    }
    return super.addDeclaration(identifier2, context, init2, destructuredInitPath, kind);
  }
};
...
var ClassNode = class extends NodeBase {
  ...
  includePath(_path, context, includeChildrenRecursively) {
    ...
    if (!this.deoptimized)
      this.applyDeoptimizations();
    this.included = true;
    (_a = this.superClass) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
    this.body.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
    for (const decorator2 of this.decorators)
      decorator2.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
    if (this.id) {
...
var ParameterVariable = class extends LocalVariable {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path) {
    if (path.length >= 2 || this.argumentsToBeDeoptimized.has(UNKNOWN_EXPRESSION) || this.deoptimizationInteractions.length >= MAX_TRACKED_INTERACTIONS || path.length === 1 && (this.deoptimizedFields.has(UnknownKey) || interaction.type === INTERACTION_CALLED && this.deoptimizedFields.has(path[0]))) {
      deoptimizeInteraction(interaction);
      return;
    }
...
var CatchBodyScope = class extends ChildScope {
  ...
  addDeclaration(identifier2, context, init2, destructuredInitPath, kind) {
    ...
    if (kind === "var") {
      ...
      const declaredVariable = this.parent.parent.addDeclaration(identifier2, context, init2, destructuredInitPath, kind);
      declaredVariable.markInitializersForDeoptimization();
      this.addHoistedVariable(name, declaredVariable);
      return declaredVariable;
    }
...
var FunctionScope = class extends ReturnValueScope {
  ...
  includeCallArguments(context, interaction) {
    ...
    if (this.argumentsVariable.included) {
      const { args } = interaction;
      for (let argumentIndex = 1; argumentIndex < args.length; argumentIndex++) {
        (_a = args[argumentIndex]) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context, false);
      }
    }
  }
  addArgumentToBeDeoptimized(argument) {
...
var RestElement = class extends NodeBase {
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init2) {
    return this.included = this.argument.includeDestructuredIfNecessary(context, getIncludedPatternPath$1(destructuredInitPath), init2) || this.included;
  }
  includePath(_path, context, includeChildrenRecursively) {
    this.included = true;
...
var FunctionNode = class extends FunctionBase {
  ...
  includePath(path, context, includeChildrenRecursively) {
    ...
    (_a = this.id) == null ? void 0 : _a.includePath(UNKNOWN_PATH, createInclusionContext());
...
var MemberExpression = class extends NodeBase {
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init2) {
    if (this.included || (this.included = destructuredInitPath.length > 0 && !context.brokenFlow && init2.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, createHasEffectsContext()))) {
      init2.includePath(destructuredInitPath, context, false);
      return true;
    }
    return false;
  ...
  includeProperties(includedPath, objectPath, context, includeChildrenRecursively) {
    ...
    if (!this.included) {
      ...
      (_a = this.variable) == null ? void 0 : _a.includePath(includedPath, context);
    }
...
var ArrayPattern = class extends NodeBase {
  ...
  deoptimizeAssignment(destructuredInitPath, init2) {
    const includedPatternPath = getIncludedPatternPath(destructuredInitPath);
    for (const element of this.elements) {
      element == null ? void 0 : element.deoptimizeAssignment(includedPatternPath, init2);
    }
  }
  ...
  hasEffectsWhenDestructuring(context, destructuredInitPath, init2) {
    const includedPatternPath = getIncludedPatternPath(destructuredInitPath);
    for (const element of this.elements) {
      if (element == null ? void 0 : element.hasEffectsWhenDestructuring(context, includedPatternPath, init2)) {
        return true;
      }
    }
    return false;
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init2) {
    ...
    for (const element of this.elements) {
      if (element) {
        ...
        included = element.includeDestructuredIfNecessary(context, includedPatternPath, init2) || included;
      }
    }
    if (included) {
      for (const element of this.elements) {
        if (element && !element.included) {
          ...
          element.includeDestructuredIfNecessary(context, includedPatternPath, init2);
        }
      }
    }
...
var ObjectPattern = class extends NodeBase {
  ...
  declare(kind, destructuredInitPath, init2) {
    const variables = [];
    for (const property2 of this.properties) {
      variables.push(...property2.declare(kind, destructuredInitPath, init2));
    }
    return variables;
  }
  deoptimizeAssignment(destructuredInitPath, init2) {
    for (const property2 of this.properties) {
      property2.deoptimizeAssignment(destructuredInitPath, init2);
    }
  }
  deoptimizePath(path) {
    if (path.length === 0) {
  ...
  hasEffectsWhenDestructuring(context, destructuredInitPath, init2) {
    for (const property2 of this.properties) {
      if (property2.hasEffectsWhenDestructuring(context, destructuredInitPath, init2))
        return true;
    }
    return false;
  }
  includeDestructuredIfNecessary(context, destructuredInitPath, init2) {
    let included = false;
    for (const property2 of this.properties) {
      included = property2.includeDestructuredIfNecessary(context, destructuredInitPath, init2) || included;
    }
    return this.included || (this.included = included);
  }
  markDeclarationReached() {
...
var AssignmentPattern = class extends NodeBase {
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init2) {
    let included = this.left.includeDestructuredIfNecessary(context, destructuredInitPath, init2) || this.included;
    if (included || (included = this.right.shouldBeIncluded(context))) {
      ...
      if (!this.left.included) {
        ...
        this.left.includeDestructuredIfNecessary(context, destructuredInitPath, init2);
      }
    }
...
var ForStatement = class extends NodeBase {
  ...
  includePath(_path, context, includeChildrenRecursively) {
    var _a, _b, _c;
    this.included = true;
    (_a = this.init) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context, includeChildrenRecursively, {
      asSingleStatement: true
    });
    (_b = this.test) == null ? void 0 : _b.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
    (_c = this.update) == null ? void 0 : _c.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
    includeLoopBody(context, this.body, includeChildrenRecursively);
  }
  render(code, options) {
    var _a, _b, _c;
...
var IfStatement = class _IfStatement extends NodeBase {
  ...
  includeRecursively(includeChildrenRecursively, context) {
    ...
    (_a = this.alternate) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
  }
...
var VariableDeclarator2 = class extends NodeBase {
  ...
  includePath(_path, context, includeChildrenRecursively) {
    ...
    if (!deoptimized)
      this.applyDeoptimizations();
    this.included = true;
    init2 == null ? void 0 : init2.includePath(EMPTY_PATH, context, includeChildrenRecursively);
    id.markDeclarationReached();
    if (includeChildrenRecursively) {
      id.includePath(EMPTY_PATH, context, includeChildrenRecursively);
    } else {
      id.includeDestructuredIfNecessary(context, EMPTY_PATH, init2 || UNDEFINED_EXPRESSION);
    }
  }
  removeAnnotations(code) {
    var _a;
...
var ObjectExpression2 = class extends NodeBase {
  ...
  includePath(path, context, includeChildrenRecursively) {
    ...
    (_a = this.protoProp) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
  }
  render(code, options, { renderedSurroundingElement } = BLANK) {
...
var Property2 = class extends MethodBase {
  ...
  deoptimizeAssignment(destructuredInitPath, init2) {
    var _a, _b;
    (_b = (_a = this.value).deoptimizeAssignment) == null ? void 0 : _b.call(_a, this.getPathInProperty(destructuredInitPath), init2);
  }
  hasEffects(context) {
    if (!this.deoptimized)
      this.applyDeoptimizations();
  ...
  hasEffectsWhenDestructuring(context, destructuredInitPath, init2) {
    var _a, _b;
    return (_b = (_a = this.value).hasEffectsWhenDestructuring) == null ? void 0 : _b.call(_a, context, this.getPathInProperty(destructuredInitPath), init2);
  }
  includeDestructuredIfNecessary(context, destructuredInitPath, init2) {
    const path = this.getPathInProperty(destructuredInitPath);
    let included = this.value.includeDestructuredIfNecessary(context, path, init2) || this.included;
    if (included || (included = this.key.hasEffects(createHasEffectsContext()))) {
      ...
      if (!this.value.included) {
        ...
        this.value.includeDestructuredIfNecessary(context, path, init2);
      }
    }
    return this.included = included;
  }
...
var ReturnStatement2 = class extends NodeBase {
  ...
  includePath(_path, context, includeChildrenRecursively) {
    var _a;
    this.included = true;
    (_a = this.argument) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
    context.brokenFlow = true;
  }
  initialise() {
    super.initialise();
...
var SwitchCase = class extends NodeBase {
  ...
  includePath(_path, context, includeChildrenRecursively) {
    var _a;
    this.included = true;
    (_a = this.test) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
    for (const node of this.consequent) {
      if (includeChildrenRecursively || node.shouldBeIncluded(context))
        node.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
    }
...
var SwitchStatement = class extends NodeBase {
  ...
  includePath(_path, context, includeChildrenRecursively) {
    ...
    for (let caseIndex = this.cases.length - 1; caseIndex >= 0; caseIndex--) {
      ...
      if (isCaseIncluded) {
        switchCase2.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
        onlyHasBrokenFlow && (onlyHasBrokenFlow = context.brokenFlow && !context.hasBreak);
        context.hasBreak = false;
        context.brokenFlow = brokenFlow;
      } else {
...
var TryStatement = class extends NodeBase {
  ...
  includePath(_path, context, includeChildrenRecursively) {
    ...
    (_b = this.finalizer) == null ? void 0 : _b.includePath(UNKNOWN_PATH, context, includeChildrenRecursively);
  }
};
var unaryOperators = {
  "!": (value) => !value,
  ...
  includePath(_path, context, includeChildrenRecursively, { asSingleStatement } = BLANK) {
    ...
    for (const declarator of this.declarations) {
      ...
      if (init2 && id.included && !init2.included && (id instanceof ObjectPattern || id instanceof ArrayPattern)) {
        init2.includePath(EMPTY_PATH, context, includeChildrenRecursively);
      }
    }
  }
  initialise() {
...
createColors();
var { bold, cyan, dim, red } = createColors({
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-UESP2RJ2.js");
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-BHW6BM22.js.map
